...
데스크톱에서는 최대 너비 `70rem`의 카드형 CV 페이지를 사용합니다. 화면 폭이 `48rem` 이하가 되면 페이지 외곽 카드 장식을 제거하고, 연락처·학력·경력·프로젝트 헤더를 한 열에 맞게 재배치합니다. `30rem` 이하에서는 좌우 여백과 프로필 링크 그리드를 추가로 축소합니다.
