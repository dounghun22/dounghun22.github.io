...
| `robots.txt` | 검색 크롤러의 공개 경로와 XML sitemap 위치 |
...
공개 특허의 외부 링크는 해당 공개번호 또는 등록번호의 Google Patents 페이지를 우선한다. 페이지의 정상 응답과 특허 식별자 일치를 확인한 뒤 영문·한국어 데이터의 URL과 링크 라벨을 함께 갱신한다. Google Patents 페이지가 없거나 확인되지 않으면 특허청, Justia, Patsnap 등 검증된 공개 기록 링크를 유지하고, 다음 현행화 때 다시 확인한다.
...
공개 페이지를 수정할 때마다 아래 항목을 변경 범위에 맞게 함께 점검·갱신합니다.

- 페이지 콘텐츠·언어 변경: title, description, canonical URL, `hreflang`, `Person` JSON-LD의 공개 사실 정보
- 이미지·프로필·외부 링크 변경: `logo`, 구조화 데이터의 image·`sameAs`, 대체 텍스트와 링크 유효성
